...
import type { ThreadsContentItem } from "./content-manager";

interface ThreadsAddContentModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSave: (item: ThreadsContentItem) => void;
  editItem?: ThreadsContentItem | null;
}
...
export function ThreadsAddContentModal({
  open,
  onOpenChange,
  onSave,
  editItem,
}: ThreadsAddContentModalProps) {
  ...
  async function handleSave() {
    ...
    try {
      const url = editItem
        ? `/api/threads/content/${editItem.id}`
        : "/api/threads/content";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px] bg-zinc-950 border-zinc-800">
        <DialogHeader>
          <DialogTitle>{editItem ? "Edit Post" : "New Threads Post"}</DialogTitle>
        </DialogHeader>
          ...
          <div className="grid gap-2">
            ...
            {isOverLimit && (
              <p className="text-xs text-red-400">
                Post exceeds {MAX_CHARS} character limit by {charCount - MAX_CHARS} characters
              </p>
            )}
          </div>
