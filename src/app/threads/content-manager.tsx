...
import { ThreadsAddContentModal } from "./add-content-modal";

export interface ThreadsContentItem {
  id: number;
  title: string;
  text: string | null;
...
const typeColors: Record<ThreadsContentItem["post_type"], string> = {
  text: "bg-fuchsia-500/10 text-fuchsia-400 border-fuchsia-500/20",
...
export function ThreadsContentManager() {
  const [items, setItems] = useState<ThreadsContentItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editItem, setEditItem] = useState<ThreadsContentItem | null>(null);
  const [dragOverColumn, setDragOverColumn] = useState<Status | null>(null);

  const fetchItems = useCallback(async () => {
    const res = await fetch("/api/threads/content");
    if (res.ok) {
  ...
  function handleEdit(item: ThreadsContentItem) {
    setEditItem(item);
    setModalOpen(true);
  }

  async function handleAdvance(item: ThreadsContentItem) {
    const next = nextStatus[item.status];
    if (!next) return;

    const res = await fetch(`/api/threads/content/${item.id}`, {
      method: "PATCH",
  ...
  async function handleDelete(item: ThreadsContentItem) {
    const res = await fetch(`/api/threads/content/${item.id}`, {
      method: "DELETE",
    });
  ...
  function handleSave(saved: ThreadsContentItem) {
    setItems((prev) => {
  ...
  async function handleDrop(e: DragEvent, targetStatus: Status) {
    ...
    const res = await fetch(`/api/threads/content/${itemId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={handleOpenNew} size="sm" className="bg-fuchsia-600 hover:bg-fuchsia-700">
          + Add Content
        </Button>
      </div>
                      ...
                      <div className="flex items-start justify-between gap-2 mb-2">
                        ...
                        <div className="flex items-center gap-1.5">
                          {charCount > 0 && (
                            <span className={cn(
                              "text-[10px] font-mono",
                              charCount > 500 ? "text-red-400" : "text-zinc-500"
                            )}>
                              {charCount}/500
                            </span>
                          )}
