...
export async function PATCH(
  ...
  const existing = await sql`SELECT * FROM threads_content WHERE id = ${id}` as unknown as Record<string, unknown>[];
  if (!existing[0]) {
...
export async function DELETE(
  ...
  const result = await sql`DELETE FROM threads_content WHERE id = ${id} RETURNING id` as unknown as Record<string, unknown>[];
